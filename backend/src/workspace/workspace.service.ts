...
export class WorkspaceService {
  constructor(
    ...
    @Optional() private calendarSync: CalendarSyncService,
  ) {}
  ...
  async getSummary(userId: string, orgId: string) {
    ...
    const visArgs   = (extra: any[]) => deptId ? [userId, userId, deptId, ...extra] : [userId, userId, ...extra];
    ...
    const [today, pending, overdue, assigned] = await Promise.all([
      this.prisma.$queryRawUnsafe<any[]>(
        `SELECT COUNT(*) AS cnt FROM workspace_tasks t WHERE t.organizationId = ? AND ${visOr} AND t.dueDate >= ? AND t.dueDate < ?`,
        orgId, ...visArgs([todayStart, todayEnd])),
      this.prisma.$queryRawUnsafe<any[]>(
        `SELECT COUNT(*) AS cnt FROM workspace_tasks t WHERE t.organizationId = ? AND ${visOr} AND t.status IN ('todo','in_progress')`,
        orgId, ...visArgs([])),
      this.prisma.$queryRawUnsafe<any[]>(
        `SELECT COUNT(*) AS cnt FROM workspace_tasks t WHERE t.organizationId = ? AND ${visOr} AND t.dueDate < ? AND t.status != 'done'`,
        orgId, ...visArgs([todayStart])),
      this.prisma.$queryRawUnsafe<any[]>(
        `SELECT COUNT(*) AS cnt FROM workspace_tasks t WHERE t.organizationId = ? AND ${assnOr} AND t.status != 'done'`,
        orgId, ...assnArgs),
    ]);
    ...
    const n = (rows: any[]) => Number(rows[0]?.cnt ?? 0);
    return {
      todayTasks:   n(today),
  ...
  async getCalendarDots(userId: string, orgId: string, year: number, month: number) {
    const user = await this.prisma.user.findUnique({
      where: { id: userId }, select: { departmentId: true },
    });
    const deptId = user?.departmentId;
    ...
    const rows: any[] = await this.prisma.$queryRawUnsafe(
      `SELECT t.dueDate, t.priority FROM workspace_tasks t
       WHERE t.organizationId = ? AND t.dueDate >= ? AND t.dueDate < ? AND t.status != 'done' AND ${orClause}`,
      orgId, start, end, ...orArgs,
    );
  ...
  async getTasks(userId: string, orgId: string, filter?: string, date?: string) {
    const user = await this.prisma.user.findUnique({
      where: { id: userId }, select: { departmentId: true },
    });
    const deptId = user?.departmentId;
  ...
  async createTask(userId: string, orgId: string, body: any) {
    ...
    const rows: any[] = await this.prisma.$queryRawUnsafe(`${TASK_SQL} WHERE t.id = ?`, id);
    ...
    if (departmentId) {
      ...
      await Promise.all(
        members.map(m => this.notifications.create(m.id, orgId, {
  ...
  async updateTask(id: string, userId: string, orgId: string, body: any) {
    const existing: any[] = await this.prisma.$queryRawUnsafe(
      `SELECT id, assignedById, assignedToId, departmentId, title
       FROM workspace_tasks WHERE id = ? AND organizationId = ?`,
      id, orgId,
    );
    if (!existing.length) throw new NotFoundException('Task not found');
    ...
    if (body.pinned       !== undefined) { setClauses.push('pinned = ?');       setParams.push(body.pinned ? 1 : 0); }
    if (body.dueDate      !== undefined) { setClauses.push('dueDate = ?');      setParams.push(body.dueDate    ? new Date(body.dueDate)    : null); }
    if (body.reminderAt   !== undefined) { setClauses.push('reminderAt = ?');   setParams.push(body.reminderAt ? new Date(body.reminderAt) : null); }
    if (body.assignedToId !== undefined) { setClauses.push('assignedToId = ?'); setParams.push(body.assignedToId ?? null); }
    if (body.departmentId !== undefined) { setClauses.push('departmentId = ?'); setParams.push(body.departmentId ?? null); }

    if (setClauses.length > 0) {
      setClauses.push('updatedAt = NOW()');
      await this.prisma.$executeRawUnsafe(
        `UPDATE workspace_tasks SET ${setClauses.join(', ')} WHERE id = ?`,
        ...setParams, id,
      );
    }
  ...
  async deleteTask(id: string, userId: string, orgId: string) {
    const rows: any[] = await this.prisma.$queryRawUnsafe(
      `SELECT id, assignedById FROM workspace_tasks WHERE id = ? AND organizationId = ?`,
      id, orgId,
    );
    if (!rows.length) throw new NotFoundException('Task not found');
    if (rows[0].assignedById !== userId) throw new ForbiddenException('Only the creator can delete this task');

    // Calendar sync — fire and forget before delete so we still have the task ID
  ...
  // ── Notes (workspaceNote has all generated types — use Prisma directly) ────

  async getNotes(userId: string, orgId: string) {
  ...
  async createNote(userId: string, orgId: string, body: any) {
  ...
  async updateNote(id: string, userId: string, orgId: string, body: any) {
  ...
  async deleteNote(id: string, userId: string, orgId: string) {
    const note = await this.prisma.workspaceNote.findFirst({
      where: { id, organizationId: orgId, createdById: userId },
    });
    if (!note) throw new NotFoundException('Note not found');
