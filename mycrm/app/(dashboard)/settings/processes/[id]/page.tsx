...
function mapApiStageToLocal(s: Record<string, unknown>): Stage {
  const actions = (s.actions as Record<string, unknown>) ?? {};
  return {
    id: (s.id as string) ?? generateId(),
    name: (s.name as string) ?? "",
    assigneeType: (s.assigneeType as string) ?? "Role",
    ...
    onApprove: (s.onApprove as string) ?? "END",
    onReject: (s.onReject as string) ?? "END",
    onRequestInfo: (s.onRequestInfo as string) ?? "",
  };
}
...
export default function EditBlueprintPage() {
  ...
  useEffect(() => {
    async function fetchBlueprint() {
      try {
        setLoading(true);
        const res = await api.get(`/process/blueprints/${params.id}`);
        const data = res.data;
        setBlueprintName(data.name ?? "");
        setName(data.name ?? "");
        setDescription(data.description ?? "");
        setModuleId(data.moduleId ?? data.module?.id ?? "");
        setTriggerField(data.triggerField ?? "");
        setTriggerValue(data.triggerValue ?? "");
        setIsActive(data.isActive ?? true);
        const loadedStages = Array.isArray(data.stages) && data.stages.length > 0
          ? [...data.stages]
              .sort((a: Record<string, unknown>, b: Record<string, unknown>) =>
                ((a.order as number) ?? 0) - ((b.order as number) ?? 0)
              )
              .map((s: Record<string, unknown>) => mapApiStageToLocal(s))
          : [createDefaultStage()];
  ...
  async function handleSave() {
    ...
    setSaving(true);
    try {
      await api.patch(`/process/blueprints/${params.id}`, {
        name,
        description,
              ...
              <CardContent className="space-y-4">
                ...
                <div className="space-y-1">
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      updateStage(stage.id, {
                        slaDays: e.target.value === "" ? "" : Number(e.target.value),
                      })
                    }
                    placeholder="e.g. 3"
                    className="w-32"
