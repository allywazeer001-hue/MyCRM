...
import {
  ...
  AlertTriangle, X, Palette, LayoutGrid, Blocks, FileText, Menu as MenuIcon, Bell,
} from "lucide-react";

// ── Shared tab shell ────────────────────────────────────────────────────────
// Portal Settings, Portal Users, Portal Appearance, and Portal Builder used to
// be separate nav links; they're consolidated here as tabs of one page since
// they're all "everything about the portal" from the user's point of view.
// (The bigger multi-portal "Portal Builder Studio" at /apps/portal-builder is
// a distinct top-level app reachable from the App Switcher, not folded in here.)

type TabKey = "modules" | "users" | "appearance" | "builder";
...
const BUILDER_COLOR_MAP: Record<string, { bg: string; icon: string }> = {
...
function BuilderTab() {
  ...
  return (
    <div className="space-y-6">
      <p className="text-sm text-gray-500">
        Design and customise your portal experience — pages, navigation menus, and notifications.
      </p>

      <div className="grid gap-4">
        {BUILDER_SECTIONS.map(({ href, icon: Icon, color, title, description }) => {
          const c = BUILDER_COLOR_MAP[color];
          return (
            <button
              key={href}
