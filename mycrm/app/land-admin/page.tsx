...
function Field({ label, hint, children }: { label: string; hint?: string; children: React.ReactNode }) {
  return (
    <div className="space-y-1.5">
      <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wider block">{label}</label>
...
const INPUT = "w-full bg-secondary border border-border text-foreground text-sm rounded-xl px-3.5 py-2.5 placeholder:text-muted-foreground/50 focus:border-ring focus:outline-none focus:ring-2 focus:ring-ring/15 transition-colors";
    ...
    <Field label={label} hint={hint}>
      <div
        ...
        className={[
          ...
          dragging ? "border-ring bg-ring/5" : "border-border hover:border-foreground/20 hover:bg-accent",
        ].join(" ")}
      >
        {value ? (
          ...
          <div className="flex flex-col items-center justify-center gap-2 py-7 px-4 text-center">
            <svg className="w-7 h-7 text-muted-foreground/40" fill="none" viewBox="0 0 24 24">
              <path d="M4 16l4-4 4 4 4-6 4 6M4 20h16M12 4v8" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
            </svg>
            <p className="text-muted-foreground/70 text-xs">{uploading ? "Uploading…" : "Click or drag & drop"}</p>
          </div>
        )}
        {uploading && (
      ...
      {value && !uploading && (
        <button onClick={(e) => { e.stopPropagation(); onChange(""); }} className="text-xs text-muted-foreground/70 hover:text-red-500 transition-colors mt-1.5">
          Remove
        </button>
      )}
...
// ── Live Preview ──────────────────────────────────────────────────────────────
function LivePreview({ config, viewport }: { config: LandingConfig; viewport: "desktop" | "mobile" }) {
  const accent = config.accentColor || DEFAULTS.accentColor;
  const bg = config.bgColor || DEFAULTS.bgColor;
  const isMobile = viewport === "mobile";
  const hasImages = !!(config.image1Url || config.image2Url);

  const frame = (
    <div className="rounded-b-2xl overflow-hidden flex flex-col flex-1 min-h-0" style={{ background: bg }}>
      <div className="flex-1 overflow-auto scrollbar-hide p-4 space-y-4">
        {/* Nav */}
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-1.5">
            <div className="w-5 h-5 rounded-md flex items-center justify-center text-white shrink-0" style={{ background: accent }}>
              <LogoMark size={10} />
            </div>
            {!isMobile && <span className="text-white font-bold text-[11px]">{BRAND.name}</span>}
          </div>
          {isMobile ? (
            <svg className="w-4 h-4 text-white/50" fill="none" viewBox="0 0 24 24">
        ...
        {config.heroBadgeVisible && (
          <div className="flex justify-center">
            <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[9px] font-semibold" style={{ background: `${accent}1a`, border: `1px solid ${accent}33`, color: accent }}>
              <span className="w-1 h-1 rounded-full animate-pulse shrink-0" style={{ background: accent }} />
        ...
        {/* Hero */}
        <div className={!isMobile && hasImages ? "flex items-center gap-4" : ""}>
          <div className={[
            "min-w-0",
            !isMobile && hasImages ? "flex-1" : "",
            config.heroAlign === "center" || (isMobile && !config.heroAlign) ? "text-center" : "text-left",
            isMobile ? "text-center" : "",
          ].join(" ")}>
            <h1 className="text-white font-extrabold text-sm leading-snug whitespace-pre-line mb-2">
              {config.heroTitle}
            </h1>
            <p className="text-white/40 text-[10px] leading-relaxed mb-4 max-w-[240px]" style={{ margin: (config.heroAlign === "center" || isMobile) ? "0 auto 16px" : undefined }}>
              {config.heroSubtitle}
            </p>
            <div className={["flex gap-1.5 flex-wrap", (config.heroAlign === "center" || isMobile) ? "justify-center" : ""].join(" ")}>
          ...
          {/* Desktop: images sit beside the hero text */}
          {!isMobile && hasImages && (
            <div className="flex-1 grid grid-cols-2 gap-2 min-w-0">
              {config.image1Url && (
                <div className="h-20 rounded-lg overflow-hidden border border-white/10">
                  {/* eslint-disable-next-line @next/next/no-img-element */}
        ...
        {/* Mobile: images stack below the hero text */}
        {isMobile && hasImages && (
        ...
        <div>
          ...
          <div className={isMobile ? "grid grid-cols-2 gap-1.5" : "grid grid-cols-4 gap-1.5"}>
  ...
  if (isMobile) {
    // Phone bezel — narrower, centered, with a notch and home-indicator bar.
    return (
      <div className="h-full flex items-start justify-center pt-1">
        <div className="w-[240px] rounded-[28px] border-4 border-neutral-800 bg-neutral-800 shadow-xl flex flex-col" style={{ height: 500 }}>
          <div className="flex justify-center py-1.5 shrink-0">
            <div className="w-16 h-3.5 rounded-full bg-neutral-900" />
          </div>
          {frame}
          <div className="flex justify-center py-1.5 shrink-0">
            <div className="w-20 h-1 rounded-full bg-neutral-600" />
          </div>
        </div>
      </div>
    );
  ...
  // Desktop — browser-style frame with traffic lights + a fake URL pill.
  return (
    <div className="rounded-2xl overflow-hidden border border-border h-full flex flex-col" style={{ minHeight: 480 }}>
      <div className="px-3 py-2 bg-neutral-800 flex items-center gap-3 shrink-0">
        <div className="flex gap-1.5 shrink-0">
          <span className="w-2 h-2 rounded-full bg-red-400/80" />
          <span className="w-2 h-2 rounded-full bg-amber-400/80" />
          <span className="w-2 h-2 rounded-full bg-emerald-400/80" />
        </div>
        <div className="flex-1 bg-white/10 rounded-md px-2.5 py-1 text-[9px] text-white/40 text-center truncate">
...
export default function LandAdminPage() {
  ...
  return (
    <div className="h-full bg-background text-foreground flex flex-col overflow-hidden">
      ...
      {/* ── Top bar ── */}
      <header className="h-12 border-b border-border flex items-center justify-between px-5 shrink-0 bg-background/95 backdrop-blur-sm z-20">
        <div className="flex items-center gap-3">
          <div className="w-7 h-7 bg-brand rounded-lg flex items-center justify-center text-white">
            <LogoMark size={14} />
          </div>
          <div className="flex items-center gap-2">
            <Link href="/dashboard" className="text-muted-foreground hover:text-foreground text-sm font-medium transition-colors">{BRAND.name}</Link>
            <span className="text-muted-foreground/40 text-sm">/</span>
            <span className="text-foreground text-sm font-semibold">Landing Page</span>
          </div>
          {hasUnsaved && (
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        {/* ── Section sidebar ── */}
        <aside className="w-44 border-r border-border flex flex-col shrink-0 py-3 gap-0.5 px-2">
          {SECTIONS.map((s) => (
            <button
              key={s.id}
                    ...
                    {ACCENT_PRESETS.map((p) => (
                      <button key={p.value} onClick={() => set("accentColor", p.value)} title={p.label}
                        ...
                        <div className="w-8 h-8 rounded-xl transition-transform group-hover:scale-110"
                          style={{ background: p.value, boxShadow: config.accentColor === p.value ? `0 0 0 2px hsl(var(--background)), 0 0 0 4px ${p.value}` : "none" }} />
                        <span className="text-[9px] text-muted-foreground/70 group-hover:text-foreground transition-colors">{p.label}</span>
                      </button>
                    ))}
                  </div>
                  <Field label="Custom Color">
                    <div className="flex items-center gap-2.5">
                      <input type="color" value={config.accentColor} onChange={(e) => set("accentColor", e.target.value)}
                        className="w-10 h-10 rounded-xl border border-border bg-transparent cursor-pointer p-0.5 flex-shrink-0" />
                  ...
                  <div className="flex flex-wrap gap-2.5 mb-4">
                    {BG_PRESETS.map((p) => (
                      <button key={p.value} onClick={() => set("bgColor", p.value)} title={p.label}
                        className="flex flex-col items-center gap-1 group">
                        <div className="w-8 h-8 rounded-xl border border-border transition-transform group-hover:scale-110"
                          style={{ background: p.value, boxShadow: config.bgColor === p.value ? `0 0 0 2px hsl(var(--background)), 0 0 0 4px hsl(var(--ring))` : "none" }} />
                        <span className="text-[9px] text-muted-foreground/70 group-hover:text-foreground transition-colors">{p.label}</span>
                      </button>
                    ))}
                  </div>
                  ...
                  <Group title="Banner">
                    ...
                    {announcement.isActive && announcement.message && (
                      <div className="rounded-xl overflow-hidden border border-border">
                        <p className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground/70 px-3 pt-2.5 pb-1.5">Preview</p>
                    ...
                    <div className="flex gap-2.5">
                      ...
                      <button onClick={unpublishAnnouncement} disabled={unpublishing || !announcement.isActive}
                        className="px-4 py-2.5 rounded-xl text-sm font-semibold border border-border text-muted-foreground hover:text-foreground hover:border-foreground/25 disabled:opacity-40 transition-all">
        ...
        {/* ── Live preview ── */}
        <div className="w-[400px] border-l border-border p-4 shrink-0 overflow-y-auto scrollbar-hide">
          <div className="flex items-center justify-between mb-3">
            <span className="text-xs font-bold uppercase tracking-widest text-muted-foreground/70">Preview</span>
            <a href="/" target="_blank" rel="noopener noreferrer" className="text-[10px] text-muted-foreground/70 hover:text-foreground transition-colors flex items-center gap-1">
              Open full
          ...
          {/* Device toggle */}
          <div className="flex items-center bg-secondary rounded-lg p-0.5 gap-0.5 mb-3 w-fit">
            {([
              { key: "desktop" as const, label: "Desktop", icon: (
                <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24"><rect x="3" y="4" width="18" height="12" rx="1.5" stroke="currentColor" strokeWidth="1.8"/><path d="M8 20h8M12 16v4" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round"/></svg>
              ) },
              { key: "mobile" as const, label: "Mobile", icon: (
                <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24"><rect x="7" y="2" width="10" height="20" rx="2.5" stroke="currentColor" strokeWidth="1.8"/><path d="M11 19h2" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round"/></svg>
              ) },
            ]).map((d) => (
              <button
                key={d.key}
                onClick={() => setPreviewViewport(d.key)}
                className={[
                  "flex items-center gap-1.5 px-3 py-1.5 rounded-md text-xs font-medium transition-all",
                  previewViewport === d.key ? "bg-background shadow-sm text-foreground" : "text-muted-foreground hover:text-foreground",
                ].join(" ")}
              >
                {d.icon}
                {d.label}
          ...
          <LivePreview config={config} viewport={previewViewport} />
        </div>
