...
import {
  FileText, Users, FolderOpen, FolderInput,
  Plus, Settings, Palette, ChevronDown, Menu, X,
} from "lucide-react";

// ── Nav items ─────────────────────────────────────────────────────────────────

const NAV = [
  { href: "/cf",                label: "My Forms",       Icon: FileText,    key: "forms",   desc: "Forms you created"     },
  { href: "/cf/shared",         label: "Shared Forms",   Icon: Users,       key: "users",   desc: "Shared with you"       },
  { href: "/cf/folders",        label: "My Folders",     Icon: FolderOpen,  key: "folder",  desc: "Organize your forms"   },
  { href: "/cf/shared-folders", label: "Shared Folders", Icon: FolderInput, key: "sfolder", desc: "Shared collections"    },
];
...
export default function CloudFormsLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    try {
      ...
      if (parts.length === 3) {
        ...
        if (payload.exp && payload.exp * 1000 < Date.now()) { router.push("/login"); return; }
      }
    } catch {}
    setReady(true);
  }, [router]);
  ...
  const isFullscreen =
    pathname?.includes("/builder") || pathname?.includes("/permissions");

  // ── Builder / permissions → full-screen + floating CF badge ──────────────
  if (isFullscreen) {
    return (
      <>
        {children}
  ...
  // ── Main CF app shell ─────────────────────────────────────────────────────
  return (
    <div className="h-screen flex flex-col overflow-hidden" style={{ background: "#eef0f5" }}>
      ...
      <div className="flex flex-1 overflow-hidden gap-2 p-2 lg:gap-3 lg:p-3">
        ...
        {/* ── Left nav panel ─────────────────────────────────────────────── */}
        {/*   xl (≥1280): 2-col icon-grid with labels (150px)               */}
        {/*   lg (1024-1279): icon-only single column (52px)                */}
        {/*   < lg: hidden, shown via mobile overlay                        */}
        <aside
          className="hidden lg:flex flex-col rounded-2xl bg-white shrink-0 overflow-hidden lg:w-14 xl:w-[150px]"
          style={{ boxShadow: "0 2px 10px rgba(0,0,0,0.07)", border: "1px solid rgba(255,255,255,0.8)" }}
        >
          {/* Panel title — xl only */}
          <div
            className="px-3 pt-3 pb-2 shrink-0 hidden xl:block"
          ...
          <div className="p-1.5 flex-1 content-start grid gap-1.5 grid-cols-1 xl:grid-cols-2">
            {NAV.map(({ href, label, Icon, key }) => {
              const isActive =
                pathname === href || (href !== "/cf" && pathname?.startsWith(href));
              return (
                <Link key={href} href={href} title={label}>
                  <div
                    className={cn(
                      "cf-nav-card flex flex-col items-center justify-center rounded-xl cursor-pointer transition-all duration-200 select-none py-2.5",
                  ...
                  >
                    ...
                    <span className="hidden xl:block text-[9px] font-semibold text-center leading-tight px-0.5 mt-1.5">
                      {label}
                    </span>
                  </div>
                </Link>
          ...
          <div className="p-1.5 shrink-0" style={{ borderTop: "1px solid #f1f5f9" }}>
            <Link href="/cf?new=1">
              <button
                className="cf-new-form-btn w-full flex items-center justify-center gap-1.5 py-2 rounded-xl text-white text-[11px] font-bold transition-all"
                style={{
                  background: "linear-gradient(135deg, #7c3aed, #4f46e5)",
        ...
        {/* ── Right panel — xl (≥1280) only ──────────────────────────────── */}
        <aside
          className="hidden xl:flex w-[188px] flex-col rounded-2xl bg-white shrink-0 overflow-hidden"
          style={{ boxShadow: "0 2px 10px rgba(0,0,0,0.07)", border: "1px solid rgba(255,255,255,0.8)" }}
        >
          <div className="px-3 pt-3 pb-2 shrink-0" style={{ borderBottom: "1px solid #f1f5f9" }}>
            <p className="text-[10.5px] font-bold text-slate-700">Form Options</p>
            <p className="text-[9px] text-slate-400 mt-0.5">Quick tools &amp; actions</p>
          </div>
          ...
          <div className="flex-1 overflow-auto p-2.5 space-y-4">
            ...
            <div>
              ...
              <div className="space-y-1">
                {[
                  { label: "Create Form",   icon: "📝", href: "/cf?new=1"          },
                  { label: "Templates",     icon: "📋", href: "/cf/templates"      },
                  { label: "View Reports",  icon: "📊", href: "/cf/responses"      },
                  { label: "Integrations",  icon: "⚡", href: "/cf/integrations"   },
                ].map(item => (
                  <Link key={item.label} href={item.href}>
                    <div className="cf-quick-action flex items-center gap-2 px-2.5 py-1.5 rounded-lg cursor-pointer transition-all">
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-1">
                {[
                  ...
                  <button
                    key={item.label}
                    className="cf-template-opt flex flex-col items-center gap-1 py-2 px-1 rounded-xl transition-all"
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-1.5">
                {["#7c3aed","#4f46e5","#2563eb","#0891b2","#059669","#d97706","#dc2626","#db2777"].map(c => (
                  <button
                    key={c}
        ...
        <>
          ...
          >
            ...
            <div className="p-3 shrink-0" style={{ borderTop: "1px solid #f1f5f9" }}>
              <Link href="/cf?new=1" onClick={() => setMobileNav(false)}>
                <button
